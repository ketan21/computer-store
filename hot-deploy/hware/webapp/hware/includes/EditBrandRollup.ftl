...
<div class="screenlet">
    <div class="screenlet-title-bar">
        <h3>Brands of [${parameters.showProductCategoryId}]</h3>
    </div>
                ...
                <tr class="header-row">
                    ...
                    <#if productCategoryRollup.thruDate?exists && nowTimestamp.after(productCategoryRollup.getTimestamp("thruDate"))><#assign hasExpired = true></#if>
                        <tr valign="middle"<#if rowClass == "1"> class="alternate-row"</#if>>
                            <td><#if curCategory?has_content><a href="<@ofbizUrl>ViewBrandProducts?productCategoryId=${curCategory.productCategoryId}&parentProductCategoryId=${productCategoryRollup.parentProductCategoryId?if_exists}</@ofbizUrl>" class="buttontext">${curCategory.description?if_exists} [${curCategory.productCategoryId}]</a></#if></td>
                            <td <#if hasntStarted>style="color: red"</#if>>${productCategoryRollup.fromDate}</td>
                            <td align="center">
                                <#if curCategory?has_content><a href="<@ofbizUrl>EditBrand?productCategoryId=${curCategory.productCategoryId}&parentProductCategoryId=${productCategoryRollup.parentProductCategoryId}</@ofbizUrl>" class="buttontext">${uiLabelMap.CommonEdit}</a></#if>
                                <input type="hidden" name="showProductCategoryId" value="${productCategoryId}" />
