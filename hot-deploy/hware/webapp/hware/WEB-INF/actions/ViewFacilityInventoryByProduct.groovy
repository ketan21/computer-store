...
stockProductStoreId = request.getParameter("stockProductStoreId");
...
if (action){
    ...
    if ((stockProductStoreId) && (stockProductStoreId != "all"))
    {
    	productStore = delegator.findOne("ProductStore", [productStoreId : stockProductStoreId], false);
  ...
  /*  if (offsetQOHQty) {
    ...
    prods.each { oneProd ->
        ...
        if(stockProductStoreId == "all"){
        resultMap = dispatcher.runSync("getProductInventoryAndFacilitySummary", [productId : oneProd.productId, minimumStock : minimumStock, checkTime : checkTime, statusId : statusId]);
        }
        else{
          resultMap = dispatcher.runSync("getProductInventoryAndFacilitySummary", [productId : oneProd.productId, minimumStock : minimumStock, checkTime : checkTime, statusId : statusId, facilityId : facilityId]);
        }
        ...
        if (resultMap) {
            ...
            oneInventory.defaultPrice = resultMap.defultPrice;
